...
class TCPHiddenServiceEndpoint(object):
    ...
    def _connect_launched_tor(self, tor_ports):
        ...
        try:
            control_port_endpoint = clientFromString(
                self.reactor, 'tcp:127.0.0.1:%s' % control_port)
        except ValueError:
            control_port_endpoint = clientFromString(self.reactor, 'unix:%s' %
                                                     control_port)
        d = txtorcon.build_tor_connection(control_port_endpoint,
                                          build_state=False)
        return d
    ...
    def _launch_tor(self, value):
        return launch_tor(self.config,
                          self.reactor,
                          progress_updates=self.tor_proc_updates,
                          timeout=60)
    ...
    def check_local_endpoint(self, ep):
        ...
        if hasattr(ep, '_interface'):
            is_local = ep._interface.startswith(
                '127.') or ep._interface == '::1'
            if not is_local:
                return False
        return True
    ...
    def _create_listener(self, value):
        ...
        if not self.check_local_endpoint(self.tcp_endpoint):
            raise RuntimeError(
                "Endpoint doesn't appear to be a local interface.")
    ...
    def _create_onion_address(self, value):
        ...
        return TorOnionListeningPort(self.tcp_listening_port, self.onion_uri,
                                     self.public_port)
