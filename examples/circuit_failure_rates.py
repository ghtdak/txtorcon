...
class CircuitFailureWatcher(txtorcon.CircuitListenerMixin):
    ...
    def update_percent(self):
        ...
        if self.percent > 50.0:
            print 'WARNING: %02.1f percent of all routes have failed: %d failed, %d built' % (
                self.percent, self.failed_circuits, self.total_circuits)

    def information(self):
        rtn = '%02.1f%% of all circuits have failed: %d failed, %d built' % (
            self.percent, self.failed_circuits, self.total_circuits)
...
def on_shutdown(*args):
    ...
    print '  %s --failed %d --built %d' % (
        sys.argv[0], listener.failed_circuits, listener.total_circuits)
