...
def build_tor_connection(connection,
    ...
    d = endpoint.connect(TorProtocolFactory(password_function=
                                            password_function))
    if build_state:
...
class TorState(object):
    ...
    def __init__(self, protocol, bootstrap=True, write_state_diagram=False):
        ...
        waiting_s.add_transition(Transition(
            waiting_r, lambda x: x[:2] != 's ' and x[:2] != 'a ', die(
                'Expected "s " while parsing routers not "%s"')))
        waiting_s.add_transition(Transition(
            waiting_r, lambda x: x.strip() == '.', nothing))
    ...
    def close_stream(self, stream, reason='REASON_MISC', **kwargs):
        ...
        return self.queue_command('CLOSESTREAM %s %d %s' %
                                  (str(stream_id), reason, flags))
    ...
    def build_circuit(self, routers=None):
        ...
        if routers is None or routers == []:
            ...
            cmd = "EXTENDCIRCUIT 0 " + ','.join(map(lambda x: x.id_hex[1:],
                                                    routers))
