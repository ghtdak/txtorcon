...
class CircuitFailureWatcher:
    ...
    def circuit_failed(self, circuit, reason):
        log.msg('Circuit failed: %d for purpose "%s" because "%s"' %
                (circuit.id, circuit.purpose, reason))
        self.failed_circuits += 1
        percent = 100.0 * (float(self.failed_circuits) /
                           float(self.total_circuits + self.failed_circuits))
        log.msg('%02.1f%% of all circuits have failed; %d of %d' %
                (percent, self.failed_circuits, self.total_circuits))
        if percent > 50.0:
            log.warn(
                'More than half (%02.1f%%) of all circuits have failed since I started monitoring'
                % percent)
