...
class DisconnectionTests(unittest.TestCase):
    ...
    def test_disconnect_callback(self):
        ...
        self.protocol.on_disconnect.addCallback(it_was_called).addErrback(
            it_was_called)
...
OK''' % cookietmp.name)
    ...
    def test_getinfo_incremental(self):
        d = self.protocol.get_info_incremental("FOO", functools.partial(
            self.incremental_check, "bar"))
        self.send("250+FOO=")
        self.send("bar")
        self.send("bar")
        self.send(".")
    ...
    def test_getinfo_one_line(self):
        d = self.protocol.get_info("foo", functools.partial(
            self.incremental_check, "bar"))
    ...
    def test_setconf_with_space(self):
        d = self.protocol.set_conf("foo", "a value with a space").addCallback(
