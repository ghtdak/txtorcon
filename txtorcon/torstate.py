...
class TorState(object):
    ...
    def __init__(self, protocol, bootstrap=True):
        ...
        waiting_s.add_transition(Transition(
            eat_line, lambda x: x[:2] != 's ', die(
                'Expected "s " while parsing routers not "%s"')))
        waiting_s.add_transition(Transition(
            eat_line, lambda x: x.strip() == '.', nothing))
    ...
    def _bootstrap(self, arg=None):
        ...
        ns = yield self.protocol.get_info_incremental(
            'ns/all', functools.partial(strip_ok_lines,
